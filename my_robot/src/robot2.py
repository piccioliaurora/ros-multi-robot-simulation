...
class Robot2:
    ...
    def move_towards_target(self):
        ...
        #se il robot è lontano dall'obiettivo, si muove verso di esso
        if distance_to_goal > 0.5:
            #se l'angolo tra la posizione attuale del robot e l'obiettivo è significativo, ruota verso l'obiettivo
            if abs(angle_to_goal - self.theta) > 0.1:
                speed.linear.x = 0.0
                speed.angular.z = 0.3 if angle_to_goal - self.theta > 0 else -0.3
            else:
                #se l'angolo è piccolo, si muove in linea retta verso l'obiettivo
                speed.linear.x = 0.6
                speed.angular.z = 0.0
        else:
            #se è abbastanza vicino all'obiettivo, si ferma
            speed.linear.x = 0.0
            speed.angular.z = 0.0
